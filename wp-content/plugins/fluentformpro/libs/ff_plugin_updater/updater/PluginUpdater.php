...
class PluginUpdater
{
    ...
    public function __construct($config = [])
    {
        $defaults = [
            ...
            'site_url'             => '', // Canonical License site identity, injected by FluentLicensing.
            'show_check_update'    => false, // Show check update link in plugin row meta.
            'verify_updates'       => true, // Refuse update packages that are not signed by a trusted release key.
        ];
    ...
    private function initPluginUpdaterHooks()
    {
        ...
        add_action('fluentform_do_email_report_scheduled_tasks', function () {
            // Refresh from the store on the daily cron, bypassing the cache so a
            // pending release is picked up even if nobody opens the Plugins page.
            delete_transient($this->cache_key);
            $this->getVersionInfo();
        }, 100);

        // Verify the signed release manifest before WordPress unzips anything.
        // Only the policy question is answered here; whether verification is
        // possible is settled inside the filter, which fires only during an
        // actual update rather than on every request.
        if ($this->config['verify_updates']) {
            $this->registerUpdateVerifier();
        }

        if ($this->config['show_check_update']) {
            $getParam = 'fluent_sl_check_update_' . $this->config['slug'];
            $nonceAction = 'fluent_sl_check_update_' . $this->config['slug'];

            add_filter('plugin_row_meta', function ($links, $file) use ($getParam, $nonceAction) {
                if ($this->config['basename'] !== $file) {
                    return $links;
                }

                $checkUpdateUrl = wp_nonce_url(
                    admin_url('plugins.php?' . $getParam . '=' . time()),
                    $nonceAction
                );
                $row_meta = array(
                    'check_update' => '<a  style="color: #583fad;font-weight: 600;" href="' . $checkUpdateUrl . '" aria-label="' . esc_attr__('Check Update', 'fluentformpro') . '">' . esc_html__('Check Update', 'fluentformpro') . '</a>',
                );
            ...
            if (isset($_GET[$getParam])) {
                add_action('admin_init', function () use ($nonceAction) {
    ...
    /**
     * Hand the verifier the two things it cannot work out for itself: how to reach
     * the licensing API without a cache in the way, and how to drop a cached
     * response once it turns out to be stale.
     *
     * @return void
     */
    private function registerUpdateVerifier()
    {
        if (!class_exists('\\' . __NAMESPACE__ . '\\UpdateVerifier')) {
        ...
        (new UpdateVerifier(
            ...
            $this->config['version'],
            function () {
                // Uncached, so a cached response that a release has overtaken does
                // not block an update the API can still prove.
    ...
    public function pluginsApiFilter($data, $action = '', $args = null)
    {
        ...
        if (!$data) {
            return new \WP_Error('no_data', __('No data found for this plugin', 'fluentformpro'));
        }
    ...
    private function getRemoteVersionInfo()
    {
        ...
        $payload = $this->formatData([
            'item_id'          => $this->config['item_id'],
            'current_version'  => $this->config['version'],
            'site_url'         => $this->config['site_url'] ?: home_url(),
    ...
    {
        $lastSent = (int)get_option('_fluent_last_m_run', 0);

        if ($lastSent && (time() - $lastSent) < 7 * DAY_IN_SECONDS) {
            return $data;
        }

        update_option('_fluent_last_m_run', time(), 'no');
        $data['plugins'] = array_values((array)get_option('active_plugins', []));
