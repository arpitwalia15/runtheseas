...
class ActiveCampaignApi
{
	...
	public function __construct( $apiUrl, $apiKey = null )
	{
		...
		// SECURITY (PRO-38): the API key is sent to $apiUrl/admin/api.php. ActiveCampaign permits
		// self-hosted custom domains, so a fixed host allowlist is not possible, but a tampered
		// apiUrl must not be usable as an SSRF / credential-exfiltration primitive against internal
		// hosts. Reject non-http(s), invalid, and link-local targets; the requests themselves use
		// wp_safe_remote_* (which blocks private ranges). Fail closed by emptying an unsafe URL.
		if ( $this->apiUrl && ! self::isSafeApiUrl( $this->apiUrl ) ) {
			$this->apiUrl = '';
		}
	}

	protected static function isSafeApiUrl( $url )
	{
		...
		// SECURITY (PRO-38): the API key travels in the query string on every request, so require
		// https for confidentiality — SSRF host validation does not protect the transport. A legacy
		// self-hosted plaintext install can opt back in explicitly (and accepts the exposure).
		$allowHttp = (bool) apply_filters( 'fluentform/activecampaign_allow_http', false, $host );
		if ( 'https' !== $scheme && ! ( 'http' === $scheme && $allowHttp ) ) {
			return false;
		}
		...
		if ( strpos( $ip, '169.254.' ) === 0 || 'metadata.google.internal' === $host ) {
			return false;
		}
		return true;
	}
