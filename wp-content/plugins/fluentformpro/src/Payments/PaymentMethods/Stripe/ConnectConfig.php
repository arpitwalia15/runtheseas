...
class ConnectConfig
{
    ...
    public static function getConnectConfig()
    {
        ...
        $urlBase = rawurlencode(admin_url('admin.php?page=fluent_forms_settings&component=payment_settings&ff_connect_nonce=' . $hash));
    ...
    public static function verifyAuthorizeSuccess($data)
    {
        ...
        if (!$connectNonce || !wp_verify_nonce($connectNonce, 'ff_stripe_connect')) {
            echo '<div class="ff_message ff_message_error">' . esc_html__('Invalid or expired Stripe Connect request. Please start the connection again.', 'fluentformpro') . '</div>';
            return;
        }
