...
class RTS_Admin_Menu_4 {
	...
	public static function register_menu() {
		...
		RTS_Auth::page( 'rts-admin', 'Super Admin Dashboard', 'Super Admin Dashboard', 'rts_view', 'rts-super-admin', array( __CLASS__, 'render_super' ) );
	...
	private static function back( $page, $msg = '' ) { $args = array( 'page' => $page ); if ( $msg ) { $args['rts_msg'] = rawurlencode( $msg ); } wp_safe_redirect( add_query_arg( $args, admin_url( 'admin.php' ) ) ); exit; }
	...
	private static function section( $t ) { return '<h3 style="margin:22px 0 8px;padding:4px 10px;background:#FBF3DD;border:1px solid #C9A24B;display:inline-block;font-size:12px;letter-spacing:.5px">' . esc_html( $t ) . '</h3>'; }
	...
	public static function render_exec() {
		$s = RTS_Business_Logic_4::executive_summary_v2(); $k = $s['referral_coefficient']['k'] ?? 0;
		echo '<div class="wrap"><h1>Executive Dashboard — Live, real WordPress + MySQL data</h1>';
		echo self::section( '1 — GROWTH & VOLUME' ) . '<div style="display:flex;gap:12px;flex-wrap:wrap">' . self::kpi( 'Total surveys completed', $s['total_surveys_completed'] ) . self::kpi( 'Completion rate', $s['survey_completion_rate'] . '%' ) . self::kpi( 'Week-over-week growth', is_null( $s['week_over_week_growth'] ) ? '—' : $s['week_over_week_growth'] . '%', is_null( $s['week_over_week_growth'] ) ? 'not enough history yet' : '' ) . '</div>';
		echo self::section( '2 — VIRAL / REFERRAL ENGINE' ) . '<div style="display:flex;gap:12px;flex-wrap:wrap">' . self::kpi( 'Referral Coefficient (K)', $k, 'K>1 = self-sustaining' ) . self::kpi( 'Verified referrals', $s['verified_referrals_total'], 'of ' . $s['total_referrals_sent'] . ' sent' ) . self::kpi( 'Avg referrals / Founding Runner', $s['avg_referrals_per_founding_runner'] ) . '</div>';
		echo self::section( '3 — AUDIENCE COMPOSITION' ) . '<div style="display:flex;gap:12px;flex-wrap:wrap">' . self::kpi( 'Runners / non-runners', $s['runners_vs_non_runners']['runners'] . ' / ' . $s['runners_vs_non_runners']['non_runners'] ) . self::kpi( 'Avg travel party', $s['avg_travel_party_size'] ?? '—' ) . self::kpi( 'Total participants', $s['total_participants'] ) . self::kpi( 'Verified', $s['verified_participants'] ) . '</div>';
		echo '<div style="display:flex;gap:24px;flex-wrap:wrap;margin-top:12px"><div>' . self::section( 'GEOGRAPHIC DISTRIBUTION' ) . self::table( array( 'Country', 'Count' ), array_map( fn( $r ) => array( $r->country, $r->c ), $s['geographic_distribution'] ) ) . '</div><div>' . self::section( 'MARKETING SOURCE' ) . self::table( array( 'Source', 'Count' ), array_map( fn( $r ) => array( $r->marketing_source, $r->c ), $s['marketing_source_breakdown'] ) ) . '</div></div>';
		echo self::section( '4 — DEMAND & CABIN CREDIT' ) . '<div style="display:flex;gap:12px;flex-wrap:wrap">' . self::kpi( 'Cabin credit progress', $s['cabin_credits_issued'] . ' / ' . $s['cabin_credit_floor'], 'floor · target ' . $s['cabin_credit_target'] ) . self::kpi( 'Cost / Founding Runner', '—', 'pending ad-spend feed' ) . '</div>';
		$f = $s['conversion_funnel']; $pct = fn( $n ) => $f['registered'] ? round( $n / $f['registered'] * 100, 1 ) . '%' : '0%';
		echo self::section( '5 — CONVERSION FUNNEL' ) . self::table( array( 'Stage', 'Count', '% of registered' ), array( array( 'Registered', $f['registered'], '100%' ), array( 'Verified', $f['verified'], $pct( $f['verified'] ) ), array( 'Credited (Founding Runner)', $f['credited'], $pct( $f['credited'] ) ) ) );
		echo '</div>';
	}
	...
	public static function render_super() {
		...
		echo '<form method="get" action="' . esc_url( admin_url( 'admin.php' ) ) . '"><input type="hidden" name="page" value="rts-super-admin"><input type="search" name="q" value="' . esc_attr( $q ) . '" placeholder="Global search — participants, surveys, trophies, admins, audit log…" style="width:480px"> <button class="button">Search</button></form>';
		if ( strlen( $q ) >= 2 ) {
