...
foreach ( array( 'rts_cron_campaign_triggers', 'rts_cron_scheduled_reports', 'rts_cron_action_items', 'rts_cron_fr_sync', 'rtsap_run_updraft_backup' ) as $h ) { wp_clear_scheduled_hook( $h ); }
foreach ( array( 'rtsap_failed_logins_24h', 'rtsap_active_sessions', 'rtsap_auth_log_pruned' ) as $t ) { delete_transient( $t ); }
foreach ( array( 'rts_settings', 'rts_db_version', 'rts_admin_platform_db_version', 'rts_site_offline', 'rts_site_offline_at', 'rts_site_offline_by', 'rts_cron_last_campaign_triggers', 'rts_cron_last_scheduled_reports', 'rts_cron_last_action_items', 'rts_cron_last_fr_sync' ) as $o ) { delete_option( $o ); }
